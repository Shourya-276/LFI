...
// Define separate schemas for self-employed and professional
...
const professionalIncomeSchema = z.object({
  employmentType: z.literal("professional"),
  grossITRIncome: z.string().min(1, "Gross ITR income is required"),
  netITRIncome: z.string().min(1, "Net ITR income is required"),
  rentIncome: z.string().optional(),
  gstReturn: z.string().optional(),
...
// Create a discriminated union type with separate schemas
...
const IncomeDetailsForm = () => {
  ...
  // CHANGED: Updated default values to include professional
  const defaultValues: any = {
    ...
    rentIncome: application.incomeDetails?.employmentType === "salaried" 
      ...
      : (application.incomeDetails?.employmentType === "self-employed" || application.incomeDetails?.employmentType === "professional"
        ? (application.incomeDetails as any).rentIncome || ""
        : ""),
    ...
    monthlyIncentive: application.incomeDetails?.employmentType === "salaried" 
      ...
      : "",
    grossITRIncome: (application.incomeDetails?.employmentType === "self-employed" || application.incomeDetails?.employmentType === "professional")
      ? (application.incomeDetails as any).grossITRIncome || "" 
      : "",
    netITRIncome: (application.incomeDetails?.employmentType === "self-employed" || application.incomeDetails?.employmentType === "professional")
      ? (application.incomeDetails as any).netITRIncome || "" 
      : "",
    gstReturn: (application.incomeDetails?.employmentType === "self-employed" || application.incomeDetails?.employmentType === "professional")
  ...
  // CHANGED: Updated onSubmit to include professional
  const onSubmit = (data: IncomeDetailsFormValues) => {
    ...
    } else if (data.employmentType === "self-employed" || data.employmentType === "professional") {
      // Fix: Ensure existingLoans has all required fields set to non-optional values
      const processedExistingLoans = (data.existingLoans || []).map(loan => ({
        type: loan.type || "",
        emiRate: loan.emiRate || "",
  ...
  return (
    <div>
      ...
      <div className="space-y-4 mb-6">
        ...
        <Select onValueChange={handleEmploymentTypeChange} value={selectedEmploymentType || ""}>
      ...
      {/* CHANGED: Updated condition to include professional */}
      {(selectedEmploymentType === "self-employed" || selectedEmploymentType === "professional") && (
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      {/* ADDED: Show nothing for retired, housewife, student */}
      {(selectedEmploymentType === "retired" || 
        selectedEmploymentType === "housewife" || 
        selectedEmploymentType === "student") && (
        <div className="text-center py-8">
          <p className="text-gray-600">No additional income details required.</p>
          <div className="flex justify-end gap-4 mt-6">
            <Button type="button" variant="outline" onClick={handleClear}>
              Clear
            </Button>
            <Button type="button" onClick={() => saveIncomeDetails({} as any)}>
              Save & Next
            </Button>
          </div>
        </div>
